...
import com.zmy.container.CacheValueWrapper;
...
public class RedisUtils {
    ...
    public static <T> T get(String key, Class<T> cls){
        CacheValueWrapper<T> val = null;
        try {
            val = (CacheValueWrapper<T>) redisTemplate.opsForValue().get(key);
        } catch (Exception e) {
            logger.error("redis缓存操作失败", e);
            return null;
        }
            ...
            return (List<T>)list.stream().map( value -> {
                ...
                if(!cls.isInstance(val)){
                    throw new IllegalArgumentException("类型不匹配,需要序列化的class="
                            + cls.getName() + ",实际数据class=" + value.getClassName());
                }
              return val;
